...
const T_IND_PRIVACY = `1. 수집 목적: 회원 가입·관리, 구직 서비스 제공, 출퇴근·정산 관리\n2. 수집 항목: [필수] 이름, 휴대폰 번호, 이메일 / [선택] 주소, 계좌정보\n3. 보유 기간: 회원 탈퇴 시까지 (관련 법령에 따라 일부 보관)\n4. 제3자 제공: 매칭된 기업에 이름·연락처·근무 기록 제공 (매칭 목적)`

const T_IND_MARKETING = `마케팅 정보 수신 동의 (선택)\n\n- 수신 채널: 이메일, SMS/카카오 알림톡\n- 내용: 맞춤 공고 추천, 이벤트·프로모션, 서비스 업데이트\n- 수신 거부: 마이페이지 > 알림 설정에서 언제든 변경 가능\n\n※ 미동의 시에도 서비스 이용에 제한이 없습니다.`

const T_IND_BENEFITS = `개인정보 수집·이용 동의 — 공고추천·혜택 (선택)\n\n- 목적: 근무 이력·선호 지역 기반 맞춤 공고 추천, 포인트·혜택 제공\n- 항목: 근무 완료 이력, 활동 가능 지역, 선호 업종\n- 보유 기간: 동의 철회 시 즉시 삭제\n\n※ 미동의 시에도 기본 서비스 이용에 제한이 없습니다.`
...
function IndTermItem({ label, required, content, checked, onChange, expanded, onToggle }) {
  return (
    <div className="border-b border-gray-100 last:border-0">
      <div className="flex items-center gap-3 py-3">
        <button
          ...
          className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 transition-all ${
            checked ? 'bg-orange border-orange' : 'border-gray-300 hover:border-orange'
          }`}
        >
          {checked && <Check size={10} className="text-white" />}
        </button>
        <span className="flex-1 text-sm text-gray-700">
          <span className={`font-semibold mr-1 ${required ? 'text-orange' : 'text-gray-400'}`}>
            ({required ? '필수' : '선택'})
          </span>
          {label}
        </span>
      ...
      {content && expanded && (
        <div className="ml-8 mb-3 bg-gray-50 rounded-lg px-3 py-2.5 text-[11px] text-gray-400 leading-relaxed whitespace-pre-line max-h-24 overflow-y-auto border border-gray-100">
          {content}
        </div>
      )}
    </div>
...
export default function RegisterPage() {
  ...
  // Terms state (INDIVIDUAL)
  const [indTerms, setIndTerms] = useState({ age: false, service: false, privacy: false, marketing: false, benefits: false })
  const [indExpanded, setIndExpanded] = useState({ age: false, service: false, privacy: false, marketing: false, benefits: false })
  const allIndRequired = indTerms.age && indTerms.service && indTerms.privacy
  const allIndTerms    = allIndRequired && indTerms.marketing && indTerms.benefits
  function toggleIndAll() {
    const next = !allIndTerms
    setIndTerms({ age: next, service: next, privacy: next, marketing: next, benefits: next })
  }
  ...
  // ── Phone verification simulation (기업용 — 자동 인증) ──────────────────────
  ...
  // ── Phone verification simulation (개인용 — 수동 OTP 입력) ──────────────────
  function sendIndPhoneCode() {
    if (!form.phone.trim() || phoneSending || phoneCodeSent) return
    setPhoneSending(true)
    setTimeout(() => {
      setPhoneSending(false)
      setPhoneCodeSent(true)
  ...
  function confirmIndPhoneCode() {
    if (form.phoneCode.replace(/\D/g, '').length !== 6) {
      setErrors(e => ({ ...e, phoneCode: '6자리 인증번호를 입력해 주세요' }))
      return
    }
    // 데모: 어떤 6자리든 수락
    setPhoneVerified(true)
    setErrors(e => ({ ...e, phoneCode: '' }))
  }
  ...
  function validateIndividual() {
    ...
    if (!form.phone.trim()) e.phone = '휴대폰 번호를 입력해 주세요'
    else if (!phoneVerified) e.phone = '휴대폰 인증을 완료해 주세요'
    if (!form.email.trim()) e.email = '이메일을 입력해 주세요'
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) e.email = '올바른 이메일 형식이 아닙니다'
    if (!form.password)       e.password = '비밀번호를 입력해 주세요'
    else if (form.password.length < 8) e.password = '비밀번호는 8자 이상이어야 합니다'
  ...
  return (
    <div className="min-h-screen bg-offwhite flex items-start justify-center py-10 px-6">
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-lg">
        ...
        {/* Stepper */}
        {role === 'BUSINESS'   && step >= 2 && <Stepper steps={BIZ_STEPS} current={step - 2} />}
        {role === 'INDIVIDUAL' && step >= 2 && <Stepper steps={IND_STEPS} current={step - 2} />}
        ...
        <p className="text-sm text-gray-500 mb-6">
          ...
          {role === 'INDIVIDUAL' && step === 2    && '서비스 이용에 동의해 주세요.'}
          {role === 'INDIVIDUAL' && step === 3    && '개인 회원으로 가입합니다.'}
        </p>
        ...
        {/* ══ STEP 2 (INDIVIDUAL): Terms ═══════════════════════════════════════ */}
        {role === 'INDIVIDUAL' && step === 2 && (
          <div className="space-y-3">
            {/* 전체 동의 */}
            <div
              onClick={toggleIndAll}
              className="flex items-center gap-3 px-4 py-3.5 bg-navy/5 rounded-xl cursor-pointer hover:bg-navy/10 transition-colors select-none"
            >
              <div className={`w-5 h-5 rounded flex items-center justify-center shrink-0 border-2 transition-all ${allIndTerms ? 'bg-navy border-navy' : 'border-gray-300'}`}>
                {allIndTerms && <Check size={11} className="text-white" />}
              </div>
              <p className="font-bold text-navy text-sm flex-1">전체 동의</p>
              <p className="text-xs text-gray-400">필수 3개 · 선택 2개</p>
            </div>

            <div className="border border-offwhite-200 rounded-xl px-4 divide-y divide-gray-100">
              <IndTermItem
                label="만 14세 이상입니다"
                required
                content={null}
                checked={indTerms.age}
                onChange={v => setIndTerms(t => ({ ...t, age: v }))}
                expanded={indExpanded.age}
                onToggle={() => setIndExpanded(p => ({ ...p, age: !p.age }))}
              />
              <IndTermItem
                label="서비스 이용약관 동의"
                required
                content={T_SERVICE}
                checked={indTerms.service}
                onChange={v => setIndTerms(t => ({ ...t, service: v }))}
                expanded={indExpanded.service}
                onToggle={() => setIndExpanded(p => ({ ...p, service: !p.service }))}
              />
              <IndTermItem
                label="개인정보 처리방침 동의"
                required
                content={T_IND_PRIVACY}
                checked={indTerms.privacy}
                onChange={v => setIndTerms(t => ({ ...t, privacy: v }))}
                expanded={indExpanded.privacy}
                onToggle={() => setIndExpanded(p => ({ ...p, privacy: !p.privacy }))}
              />
              <IndTermItem
                label="마케팅 정보 수신 동의"
                required={false}
                content={T_IND_MARKETING}
                checked={indTerms.marketing}
                onChange={v => setIndTerms(t => ({ ...t, marketing: v }))}
                expanded={indExpanded.marketing}
                onToggle={() => setIndExpanded(p => ({ ...p, marketing: !p.marketing }))}
              />
              <IndTermItem
                label="개인정보 수집·이용 동의 (공고추천·혜택)"
                required={false}
                content={T_IND_BENEFITS}
                checked={indTerms.benefits}
                onChange={v => setIndTerms(t => ({ ...t, benefits: v }))}
                expanded={indExpanded.benefits}
                onToggle={() => setIndExpanded(p => ({ ...p, benefits: !p.benefits }))}
              />
            </div>

            <button
              disabled={!allIndRequired}
              onClick={() => setStep(3)}
              className="w-full bg-orange text-white font-bold py-3 rounded-xl hover:bg-orange-600 transition-colors disabled:opacity-40 text-sm"
            >
              다음
        ...
        {/* ══ STEP 3 (INDIVIDUAL): Info ══════════════════════════════════════ */}
        {role === 'INDIVIDUAL' && step === 3 && (
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="010-0000-0000"
                  disabled={phoneVerified || phoneSending || phoneCodeSent}
                  className={`flex-1 border rounded-xl px-4 py-2.5 text-sm focus:outline-none transition-colors disabled:bg-offwhite disabled:text-gray-400 ${
                    errors.phone ? 'border-red-300' : 'border-offwhite-200 focus:border-navy'
                  }`}
                />
                {!phoneVerified && (
                  <button
              ...
              {phoneCodeSent && !phoneVerified && (
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2 text-xs text-blue-600 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={e => {
                        setForm(f => ({ ...f, phoneCode: e.target.value.replace(/\D/g, '').slice(0, 6) }))
                        if (errors.phoneCode) setErrors(p => ({ ...p, phoneCode: '' }))
                      }}
                      placeholder="인증번호 6자리"
                      maxLength={6}
                      className={`flex-1 border rounded-xl px-4 py-2.5 text-sm focus:outline-none tracking-[0.3em] font-mono transition-colors ${
                        errors.phoneCode ? 'border-red-300' : 'border-offwhite-200 focus:border-navy'
                      }`}
                    />
                    <button
                      type="button"
                  ...
                  <FieldError msg={errors.phoneCode} />
                </div>
              )}
      ...
      <FloatToast
        ...
        message={role === 'INDIVIDUAL' ? '인증번호가 발송되었습니다' : '사업자등록번호가 인증되었습니다'}
      />
