...
import { ArrowLeft, MapPin, Bookmark, Send, Building2, Banknote, Calendar, CheckCircle2, AlertTriangle, X, Trash2 } from 'lucide-react'
...
export default function IndividualJobDetailPage() {
    ...
    async function load() {
      ...
      try {
        ...
        if (myRes.ok) {
          ...
          const found = apps.find(a => String(a.jobPostId) === String(id))
          if (found) {
            setApplied(true)
            setApplicationId(found.applicationId)
          }
        }
      } catch (e) {
        setError(e.message)
  ...
  async function loadApplicationId() {
    const res = await applicationApi.myList().catch(() => null)
    if (!res?.ok) return
    const apps = await res.json().catch(() => [])
    const found = (Array.isArray(apps) ? apps : []).find(a => String(a.jobPostId) === String(id))
    if (found) setApplicationId(found.applicationId)
  }
            ...
            <>
              ...
              {!showCancelConfirm ? (
                <button
                  ...
                  className="w-full py-2.5 rounded-xl border border-red-200 text-red-500 text-sm font-semibold hover:bg-red-50 transition-colors flex items-center justify-center gap-1.5"
                >
                  <Trash2 size={14} />
                  지원 취소
                </button>
                ...
                <div className="border border-red-200 rounded-xl p-4 bg-red-50 space-y-3">
                  <p className="text-sm font-semibold text-red-700 text-center">정말 지원을 취소하시겠습니까?</p>
                  <div className="flex gap-2">
                    <button
                      onClick={() => setShowCancelConfirm(false)}
                      disabled={cancelling}
                      className="flex-1 py-2 rounded-xl border border-offwhite-200 bg-white text-sm font-semibold text-gray-500 hover:bg-offwhite transition-colors disabled:opacity-50"
                    >
                      아니오
                    </button>
                    <button
                      ...
                      disabled={cancelling}
                      className="flex-1 py-2 rounded-xl bg-red-500 text-white text-sm font-bold hover:bg-red-600 transition-colors disabled:opacity-60 flex items-center justify-center gap-1.5"
                    >
