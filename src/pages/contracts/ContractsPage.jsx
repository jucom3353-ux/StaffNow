...
// ── 계약서 발송 모달 ────────────────────────────────────────
function calcDailyWage(startTime, endTime, breakTime, hourlyRate) {
  if (!startTime || !endTime || !hourlyRate) return ''
  const [h1, m1] = startTime.split(':').map(Number)
  const [h2, m2] = endTime.split(':').map(Number)
  const mins = (h2 * 60 + m2) - (h1 * 60 + m1) - (Number(breakTime) || 0)
  if (mins <= 0) return ''
  return String(Math.round(hourlyRate * mins / 60))
}
...
function ContractCreateModal({ onClose, onCreate }) {
  ...
  const [jobHourlyRate, setJobHourlyRate] = useState(null)
  ...
  function handleSelectJob(job) {
    ...
    setApplicants([])
    const hourlyRate = job.wageType === 'HOURLY' ? (job.wageAmount ?? null) : null
    setJobHourlyRate(hourlyRate)
    setForm(prev => ({
      ...prev,
  ...
  function patch(key, value) {
    setForm(prev => {
      ...
      if (prev.wageType === 'DAILY' && jobHourlyRate && (key === 'startTime' || key === 'endTime' || key === 'breakTime')) {
        updated.wageAmount = calcDailyWage(updated.startTime, updated.endTime, updated.breakTime, jobHourlyRate)
      }
      return updated
    })
  }

  function handleWageTypeChange(newWageType) {
    setForm(prev => {
      let newAmount = prev.wageAmount
      if (newWageType === 'HOURLY' && jobHourlyRate) {
        newAmount = String(jobHourlyRate)
      } else if (newWageType === 'DAILY' && jobHourlyRate) {
        newAmount = calcDailyWage(prev.startTime, prev.endTime, prev.breakTime, jobHourlyRate)
      } else if (newWageType === 'MONTHLY' || newWageType === 'FIXED') {
        newAmount = ''
      }
      return { ...prev, wageType: newWageType, wageAmount: newAmount }
    })
  }
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await contractApi.create({
        ...
        wageAmount:        form.wageAmount ? Number(form.wageAmount) : null,
        workLocation:      form.workLocation || null,
        startTime:         form.startTime || null,
        endTime:           form.endTime || null,
        breakTime:         form.breakTime != null ? String(form.breakTime) : null,
      })
      if (!res.ok) {
...
export default function ContractsPage() {
  ...
  async function handleCreated() {
    const res = await contractApi.myList().catch(() => null)
    if (res?.ok) {
