...
const BIZ_STATUS_KEY     = 'staffnow_admin_biz_status'
const REPORTS_STATUS_KEY = 'staffnow_admin_reports_status'
...
export default function AdminLayout() {
  ...
  const bizPending = useMemo(() => {
    const processed = loadJson(BIZ_STATUS_KEY)
    return PENDING_BUSINESSES.filter(b => !processed[b.id]).length
  }, [])

  const reportsPending = useMemo(() => {
    const statusMap = loadJson(REPORTS_STATUS_KEY)
    return ADMIN_REPORTS.filter(r => (statusMap[r.id]?.status ?? 'pending') === 'pending').length
  }, [])
